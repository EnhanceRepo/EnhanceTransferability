...
from utils.utils import DrivingDatasetDataset
import math
import scipy
import gc
import copy
...
def train(test_loader, target_layers):
    ...
    lr_mae, lr_feat, lr_attn = 0, 0.1, 1e-6

    runtimes = []
    for i, (images, labels, _) in tqdm(enumerate(test_loader)):
        start = time.time()
...
if __name__ == "__main__":
    ...
    logger = setup_logger('pred.att.feat.{0}.{1}.{2}.{3}'.format(args.model, args.train_data_name, args.test_data_name, args.eps), output_dir, 'uap.{0}.{1}.{2}.{3}'.format(args.model, args.train_data_name, args.test_data_name, args.eps))
    logger.info(args)
    ...
    pert = train(test_loader, target_layers)
