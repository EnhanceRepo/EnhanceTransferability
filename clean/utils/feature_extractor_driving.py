...
class FeatureExtractor(object):
    ...
    def __init__(self, model, model_name, type='gduap'):
        ...
        # selected shallow layer: first convolution layer fo the network.
        if 'dave2v1' in model_name:
            self.features_handles.append(self.model.conv1.register_forward_hook(self._hook_fn_gduap))
            self.features_handles.append(self.model.conv1.register_backward_hook(self._hook_gn))
        elif 'dave2v2' in model_name:
            self.features_handles.append(self.model.conv2.register_forward_hook(self._hook_fn_gduap))
            self.features_handles.append(self.model.conv2.register_backward_hook(self._hook_gn))
            ...
            self.features_handles.append(self.model.conv1.register_forward_hook(self._hook_fn_gduap))
            self.features_handles.append(self.model.conv1.register_backward_hook(self._hook_gn))
        elif 'CILR' in model_name:
            self.features_handles.append(self.model.perception.layer1[0].conv1.register_forward_hook(self._hook_fn_gduap))
            self.features_handles.append(self.model.perception.layer1[0].conv1.register_backward_hook(self._hook_gn))
        elif 'CILRS' in model_name:
            self.features_handles.append(self.model.perception.layer1[0].conv1.register_forward_hook(self._hook_fn_gduap))
            self.features_handles.append(self.model.perception.layer1[0].conv1.register_backward_hook(self._hook_gn))
    ...
    def _back_prop(self, score):
        loss = score.sum()
        self.model.zero_grad()
        loss.backward(retain_graph=True)

    def _get_weights(self, score):
        self._back_prop(score)
    ...
    def run(self, _input, _class_idx=None, sort_type=None, **kwargs):
        self.loss_feature = []
        self.model.zero_grad()
        if 'CILR' in self.model_name or 'CILRS' in self.model_name:
            full_output, _ = self.model.forward_branch(_input['rgb'].cuda(), _input['ori_speed'].cuda(), _input['ori_controls'].cuda())
            output = full_output.squeeze()[0]

        else:
            output = self.model(_input)
        weights_max = self._get_weights(output)[0].detach()
        ...
        if kwargs['loss_type'] == 'abs': #default
            if kwargs['feat_type'] == 'all_feat_and_grad':
                ...
                # loss_to_maximum = torch.log(torch.sum(torch.abs(less_significant_feature)) / 2)
            else: # half_feat_and_grad or half_feat
