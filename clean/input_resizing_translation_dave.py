...
class TIFGSM(Attack):
    ...
    """
    def __init__(self, args, model, eps=8/255, alpha=2/255, iter_num=20, kernel_name='gaussian',
    ...
    def forward(self, images, labels):
        ...
        for _ in range(self.iter_num):
            ...
            outputs = self.model(self.input_diversity(adv_images, args))
    ...
    def input_diversity(self, image, args):
        ...
        padded = F.pad(resized_img, (pad_left, pad_right, pad_top, pad_bottom, 0, 0, 0, 0), "constant",
                       0)  # B,C,h,w, no padding for B & C
...
if __name__ == '__main__':
    ...
    ckpt_dir = f'./input_com_{args.model}/{args.iter_num}'
    if not os.path.exists(ckpt_dir):
        os.makedirs(ckpt_dir)
    logger = setup_logger('input.com.{0}.{1}'.format(args.model, args.eps), output_dir,
                          'input.com.{0}.{1}'.format(args.model, args.eps))
    logger.info(args)
    ...
    attack = TIFGSM(args, model, eps=args.eps, alpha=args.step_decay, iter_num=args.iter_num)
    runtimes = []
    for i, (x_val, y_val) in tqdm(enumerate(test_loader)):
    ...
    #    logger.info('transferability on model {0}, avg MAE loss ori-adv {1}, count {2}'.format(name, (losses_other[name].avg)*180/math.pi, losses_other[name].count))
